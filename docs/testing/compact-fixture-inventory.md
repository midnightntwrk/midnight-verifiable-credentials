...
The source rebuild fallback is intentional: changing a Compact source file, compiler/runtime input, lockfile, or artifact digest must not silently trust stale fixtures. The fixture cache proves only that committed artifact bytes match the manifest; it is not evidence that Compact source compiles. The source-build/drift lane continues to compile and test source independently. Scheduled/manual regeneration should produce a manifest diff
