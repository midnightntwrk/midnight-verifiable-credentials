...
- living-documentation scenarios now have a separate TypeScript BDD lane shape
  under `packages/use-cases/age-gate/scenarios/`; this is not a docs-only change class and should be
  treated as code
- the university diploma slice now has the same executable TypeScript BDD lane
  shape under `packages/use-cases/university/scenarios/`
...
- current authority-attested coverage proves request binding, packages/registry/root
