...
`credential-compact` and `credential-proofs` are executable private candidates,
while every other row remains a target rather than a claim that the package is
currently supported or published. ADR-0014 remains Proposed; this implementation records
