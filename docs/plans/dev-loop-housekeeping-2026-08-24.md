...
Implemented on PR #483 with a bounded follow-up from exact reviewed head `e804b3136a0e4e137ab1fb445c07a2d8b173b05e`. CI and the risk/docs technical reviews were clean on that input head; the three current-head state-machine findings are remediated locally and 73 focused watcher tests pass. Human review and merge remain pending and must not be recorded as complete. The user authorized this bounded housekeeping change, the existing PR against `develop`, configured review/fix cycles, and merge only after every repository safety gate is satisfied.
...
- Persist a bounded pending notification outbox before calling the void `sendUserMessage` API, promote a key to durable dedupe only after the exact watcher user-message marker appears in the active session branch, rescan markers at the final send boundary, and recover/retry pending markers safely across cadence ticks and restarts.
...
- Dispatch at most one model-triggering notification per observation; prune or clear state only after canonical current-head confirmation (including green), then immediately before a send re-read and authoritatively enrich the complete rollup and make one last canonical PR-state/raw-rollup read requiring `OPEN`, the exact expected head SHA, and an unchanged `statusCheckRollup`.
- Rotate bounded lookup priority across cadences, including reserved final-confirmation capacity, so API-heavy earlier PRs cannot indefinitely starve later red PRs.
...
| Delivery and lifecycle | Fake API/timer tests for bounded outbox-before-send, one dispatch per observation, final full-rollup enrichment, post-enrichment `OPEN`/exact-head/unchanged-raw-rollup confirmation, exact branch-marker confirmation, restart retry/recovery, deduped-red status, five-minute cadence, shutdown abort, stale callback rejection, and trust/mode guards |
| Shared lookup budget | Multi-PR and multi-cadence component coverage proves `pr list --limit 100`, at most 100 fixed-repository Actions job API calls per observation, and deterministic rotated access through final confirmation |
| Repository quality | 73 focused unit/component tests, deterministic strict TypeScript plus syntax checks in CI using the checked Pi 0.84.2 API contract, applicable `ci:lint`, lock consistency, diff check |
...
- GitHub rollups can be eventually consistent. A second exact-head read plus a final complete rollup re-read with authoritative Actions enrichment, followed by a last canonical PR-state and raw-rollup read, is required immediately before notification. That final boundary requires `OPEN`, the exact expected head SHA, and the same raw `statusCheckRollup` that was enriched, rejecting a superseded or closed PR and deferring any concurrent rollup change to the next cadence.
- Pi's `sendUserMessage` API is void and delivery can fail asynchronously. A bounded durable outbox plus exact active-branch marker count is therefore the acknowledgement boundary; at most three sends are allowed per failure key per watcher session before waiting for marker confirmation. A restart starts a new session and grants a still-pending failure key whose marker is absent a fresh three-send budget. Within a session, a real green observation or head/PR pruning clears the key and grants a later failure a fresh three-send budget.
...
- The checked-in quality-evidence catalog is entirely unmeasured: six rows are `not-run` and two are `blocked`. Its provenance still named superseded head `161d33d9`, which is not an ancestor of recreated `origin/develop` at `12cd5409`; it was truthfully rebaselined to exact `origin/develop` without changing or fabricating any metric.
...
- The lookup-fairness remediation on `d8ab589cd44e5316c658b98222979940815b9b2d`, extended through `6b7a43b0`, rotates stable PR lookup and final-confirmation priority across observations. A two-cadence component regression proves an API-heavy earlier PR cannot repeatedly consume the shared 100-call budget ahead of a later red PR.
- Final-boundary remediation through `bdb67bb68c34270c0b1229549718716b8b59bfc7` makes the post-enrichment canonical read require `OPEN`, the exact expected head SHA, and an unchanged raw `statusCheckRollup`.
- The bounded follow-up from `e804b3136a0e4e137ab1fb445c07a2d8b173b05e` rescans branch markers at the final send boundary, confirms the canonical current head before restored/green pruning, and reports a final resolved-green rollup as green without incrementing the pending count. Three deterministic component regressions raise the focused total to 73; human review and merge remain pending.
