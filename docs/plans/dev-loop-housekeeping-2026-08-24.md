...
Implemented and locally validated on PR #483; the final lookup-fairness finding on exact head `d8ab589cd44e5316c658b98222979940815b9b2d` is remediated in one bounded pass, and exact-head CI convergence must be rerun. The user authorized this bounded housekeeping change, the existing PR against `develop`, configured review/fix cycles, and merge only after every repository safety gate is satisfied.
...
- Dispatch at most one model-triggering notification per observation; immediately before that send, re-read and authoritatively enrich the complete rollup, then make one last canonical PR-state read and require `OPEN` plus the exact expected head SHA.
...
| Delivery and lifecycle | Fake API/timer tests for bounded outbox-before-send, one dispatch per observation, final full-rollup enrichment, post-enrichment `OPEN`/exact-head confirmation, exact branch-marker confirmation, restart retry/recovery, deduped-red status, five-minute cadence, shutdown abort, stale callback rejection, and trust/mode guards |
...
- GitHub rollups can be eventually consistent. A second exact-head read plus a final complete rollup re-read with authoritative Actions enrichment, followed by a last canonical `OPEN`/exact-head PR-state read, is required immediately before notification to reject a superseded or closed PR and consume a newer same-head rerun state.
- Pi's `sendUserMessage` API is void and delivery can fail asynchronously. A bounded durable outbox plus exact active-branch marker count is therefore the acknowledgement boundary; three sends are allowed per uninterrupted failure episode before waiting for marker confirmation. A real green observation or head/PR pruning ends the episode and grants a later failure a fresh three-send budget; restart also supplies a fresh bounded in-memory retry budget.
...
- The final lookup-fairness remediation on `d8ab589cd44e5316c658b98222979940815b9b2d` rotates a stable PR order across observations and preserves remaining capacity for the rotated first candidate's final confirmation. A two-cadence component regression proves an API-heavy earlier PR cannot repeatedly consume the shared 100-call budget ahead of a later red PR.
