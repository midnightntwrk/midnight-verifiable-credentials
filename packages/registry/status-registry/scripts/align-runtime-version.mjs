...
const runtimePackageJson = require.resolve('@midnight-ntwrk/compact-runtime/package.json');
const runtimePackage = JSON.parse(
  await readFile(runtimePackageJson, 'utf8'),
);
const runtimeVersion = runtimePackage.version;
const targetFile = path.join(packageRoot, 'src', 'managed', 'revocation-registry', 'contract', 'index.js');
const targetTypesFile = path.join(packageRoot, 'src', 'managed', 'revocation-registry', 'contract', 'index.d.ts');
