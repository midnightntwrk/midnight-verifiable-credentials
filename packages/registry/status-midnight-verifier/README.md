...
The package remains least privilege: it imports proof verification semantics but no writer, mutation, signer, or key-custody API. Public external proofs may bind a stable handle digest. Hidden/private same-contract results emit only a challenge-scoped subject digest. External hidden-holder evidence is accepted only through an explicitly injected `PrivateStatusCryptographicProofVerifierV1`; without that adapter it fails closed as `PRIVATE_STATUS_PROOF_UNAVAILABLE`. The private adapter receives leaf/path material as private witness input, while public results replace the raw proof digest with a challenge-, credential-binding-, and presentation-binding-scoped digest and never project the leaf or Merkle path.
