...
import {
  ...
  type AgeGateStepInsightPayload,
} from "./age-gate-step-insight.js";
import {
  type AgeGateScenarioResult,
  UseAgeGateScenario,
} from "./age-gate-scenario.js";
import {
  type HiddenHolderScenarioResult,
...
const summarizeAgeGateResult = (result: AgeGateScenarioResult) => ({
...
const summarizeHiddenHolderResult = (result: HiddenHolderScenarioResult) => ({
  approved: result.approved,
  claimDecision: result.claimDecision,
...
const runHiddenHolderScenario = (options: {
  ...
  readonly insight: Omit<AgeGateStepInsightPayload, "dto">;
  readonly execute: (scenario: UseHiddenHolderScenario) => Promise<void>;
}) =>
  Task.where(
    options.taskName,
    Interaction.where(options.interactionName, async (actor) => {
      const scenario = UseHiddenHolderScenario.from(actor);
      await options.execute(scenario);
      collectAgeGateStepInsight(actor, options.insightTitle, {
        ...options.insight,
        dto: summarizeHiddenHolderResult(scenario.lastResult()),
      });
    }),
  );
  ...
  Task.where(
    ...
    Interaction.where(
      ...
      async (actor) => {
        ...
        collectAgeGateStepInsight(actor, "Age-gate happy-path step insight", {
          request:
          ...
          dto: summarizeAgeGateResult(scenario.lastResult()),
        });
      },
    ),
  );
...
export const RunTheHiddenHolderLiveStatusHappyPath = () =>
  runHiddenHolderScenario({
    taskName: "#actor runs the hidden-holder same-contract live-status happy path",
    interactionName:
...
export const RunTheHiddenHolderExpiredAuthorityAttestationRejectedPath = () =>
  runHiddenHolderScenario({
    taskName: "#actor runs the hidden-holder expired-attestation rejection path",
    interactionName:
...
export const RunTheHiddenHolderUnsupportedAuthorityModeRejectedPath = () =>
  runHiddenHolderScenario({
    taskName: "#actor runs the hidden-holder unsupported authority-mode rejection path",
    interactionName:
