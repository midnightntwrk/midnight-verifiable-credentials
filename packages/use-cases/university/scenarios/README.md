...
- `./target/summary.json`
- `./target/summary.md`
- artifacts are regenerated by `test:all`, `test:all:proof-server`, and
  `test:all:standalone` before the Serenity HTML aggregation step
- use `summary.md` for a compact pass/fail table over scenario intent, tags,
  backend lane duration, and the first non-passing step
- use the Serenity HTML report when you need full step notes and attached DTO
  insights
