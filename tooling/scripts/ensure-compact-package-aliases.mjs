...
for (const { alias, target: targetDir } of officialCompatibilityAliases) {
  ...
  try {
    ...
    if (stat.isSymbolicLink()) {
      ...
      throw new Error(
        `[ensure-compact-package-aliases] ${alias} exists but is not a symlink; run pnpm run clean:artifacts before re-running, or remove the stale top-level shell manually if cleanup reports it as non-disposable.`,
      );
    }
  } catch (error) {
    if (!(error && typeof error === 'object' && 'code' in error && error.code === 'ENOENT')) throw error;
